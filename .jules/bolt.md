...
## 2024-05-29 - [Eliminate String Clones in Candidate Collection]
**Learning:** During expression type checking in `src/type_checker/expressions/access.rs`, when generating suggestions for missing fields or methods (using `candidates.extend(...)`), the compiler was cloning `String` objects (e.g. `n.clone()`, `keys().cloned()`) from `TypeDefinition` and traits to build a list of candidates. This caused unnecessary heap allocations on the error path (and potentially during inference) for strings only used for read-only comparisons in `find_best_match`.
**Action:** Replace `Vec<String>` with `Vec<&str>` for candidate collection. Instead of cloning strings, map over iterators to extract string slices using `.as_str()`. This eliminates the cloning overhead when searching for field and method names.

## 2026-05-28 - [Eliminate String Allocations in Operator Trait Name Resolution]
**Learning:** `operator_trait_class_name` in MIR lowering returned an owned `String` (`STRING_TYPE_NAME.to_string()` or `name.clone()`) on every binary operator lowering check across all types (`String`, custom types, scalars). Because binary operator lowering checks occur for every binary expression in every MIR body, returning string slices (`&'tc str` bound to the `TypeChecker` lifetime parameter on `LoweringContext<'tc>`) eliminates heap allocations on the binary expression lowering hot path and allows subsequent `&mut LoweringContext` reborrows.
**Action:** Return `Option<&'tc str>` from `operator_trait_class_name` and `binary_trait_class_name` rather than allocating owned `String`s, and construct mangled names with `String::with_capacity` + `push_str`.
