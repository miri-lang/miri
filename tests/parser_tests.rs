...
fn test_method_call_on_map_literal() {
    parse_test("{'a': 1}.keys()", vec![
        expression_statement(
            call(
                member(
                    map(vec![(string_literal("a"), int_literal(1))]),
                    identifier("keys")
                ),
                vec![]
            )
    ...
    parse_test("let funcs = {'a': fn(): 1, 'b': fn(): 2}", vec![
        variable_statement(vec![
            let_variable("funcs", None, opt_expr(map(vec![
                (string_literal("a"), lambda().build_lambda(expression_statement(int_literal(1)))),
                (string_literal("b"), lambda().build_lambda(expression_statement(int_literal(2)))),
            ])))
        ], MemberVisibility::Public)
    ]);
}
        ...
        variable_statement(vec![
            let_variable("config", None, opt_expr(map(vec![
                (string_literal("user"), map(vec![
                    ...
                    (string_literal("id"), int_literal(123)),
                ])),
...
fn test_tuple_index_access() {
    parse_test("print(my_tuple[1])", vec![
        expression_statement(
            call(
                identifier("print"),
                vec![index(identifier("my_tuple"), int_literal(1))]
            )
        )
    ]);
}
...
", vec![
        for_statement(
            vec![let_variable("el", None, None)],
            iter_obj(
                tuple(vec![symbol_literal("ok"), int_literal(200)])
            ),
            block(vec![expression_statement(call(identifier("print"), vec![identifier("el")]))])
        )
    ]);
...
fn test_single_item_tuple() {
    parse_test("let num = (42)", vec![
        variable_statement(vec![
            let_variable("num", None, opt_expr(tuple(vec![int_literal(42)])))
        ], MemberVisibility::Public)
    ]);
}
    ...
    parse_test("let num = (42 + 100)", vec![
        variable_statement(vec![
            let_variable("num", None, opt_expr(
                binary(int_literal(42), BinaryOp::Add, int_literal(100))
            ))
        ], MemberVisibility::Public)
    ]);
}
...
fn test_single_item_tuple_with_index() {
    parse_test("let num = (obj[0])", vec![
        variable_statement(vec![
            let_variable("num", None, opt_expr(tuple(vec![
                index(identifier("obj"), int_literal(0))
            ])))
        ], MemberVisibility::Public)
    ]);
}
...
", vec![
        for_statement(
            vec![let_variable("el", None, None)],
            iter_obj(set(vec![
                int_literal(1), int_literal(2), int_literal(3)
            ])),
            block(vec![expression_statement(call(identifier("print"), vec![identifier("el")]))])
        )
    ]);
...
fn test_method_call_on_set_literal() {
    parse_test("{1, 2, 3}.len()", vec![
        expression_statement(
            call(
                member(
                    set(vec![int_literal(1), int_literal(2), int_literal(3)]),
                    identifier("len")
                ),
                vec![]
            )
...
fn test_map_of_values_similar_to_set() {
    parse_test("let funcs = {func1(): 1, func2(): 2}", vec![
        variable_statement(vec![
            let_variable("funcs", None, opt_expr(map(vec![
                (call(identifier("func1"), vec![]), int_literal(1)),
                (call(identifier("func2"), vec![]), int_literal(2)),
            ])))
        ], MemberVisibility::Public)
    ]);
}
...
", vec![
        variable_statement(vec![
            let_variable("data", None, opt_expr(set(vec![
                ...
                map(vec![
                    (string_literal("id"), int_literal(1)),
                    (string_literal("name"), string_literal("John")),
                ]),
                map(vec![
                    (string_literal("id"), int_literal(2)),
