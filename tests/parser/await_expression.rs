...
fn test_await_expression() {
    parser_test("await some_future()", vec![
...
fn test_await_precedence_with_member_access() {
    ...
    parser_test("await future.get_value()", vec![
        expression_statement(
            unary(
                UnaryOp::Await,
                call(
...
fn test_await_in_variable_assignment() {
    parser_test("let result = await get_data()", vec![
...
fn test_chained_await_expression() {
    ...
    parser_test("await await future", vec![
        expression_statement(
            unary(
                UnaryOp::Await,
                unary(
...
fn test_await_precedence_with_binary_expression() {
    ...
    parser_test("await future + 1", vec![
        expression_statement(
            binary(
                unary(
                    UnaryOp::Await,
...
fn test_await_in_function_argument() {
    parser_test("my_func(await get_data())", vec![
        expression_statement(
            call(
                identifier("my_func"),
                vec![
...
fn test_await_in_conditional_expression() {
    parser_test("true if await is_ready() else false", vec![
        expression_statement(
            if_conditional(
                boolean_literal(true),
                unary(
                    UnaryOp::Await,
                    call(identifier("is_ready"), vec![])
                ),
