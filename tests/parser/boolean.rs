...
fn test_boolean_in_function_call() {
    parser_test("my_func(true, false)", vec![
        expression_statement(
            call(
                identifier("my_func"),
                vec![
...
fn test_boolean_as_method_call_target() {
    ...
    parser_test("false.to_string()", vec![
        expression_statement(
            call(
                member(
                    boolean_literal(false),
...
fn test_case_sensitive_booleans() {
    ...
    parser_test("let is_ok = True", vec![
        variable_statement(vec![
            let_variable(
                "is_ok",
                None,
                opt_expr(identifier("True"))
            )
        ], MemberVisibility::Public)
    ]);
}
