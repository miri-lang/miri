...
pub fn variable_declaration_test(input: &str, expected: Vec<VariableDeclaration>, visibility: MemberVisibility) {
...
pub fn if_statement_test(input: &str, condition: Expression, then_block: Statement, else_block: Option<Statement>, if_statement_type: IfStatementType) {
    parser_test(input, vec![
        Statement::If(Box::new(condition), Box::new(then_block), else_block.map(Box::new), if_statement_type)
    ]);
}

pub fn combined_if_unless_test(input: &str, condition: Expression, then_block: Statement, else_block: Option<Statement>) {
    if_statement_test(input, condition.clone(), then_block.clone(), else_block.clone(), IfStatementType::If);
    if_statement_test(input.replace("if", "unless").as_str(), condition, then_block, else_block, IfStatementType::Unless);
}
...
pub fn while_expression_test(input: &str, condition: Expression, then_block: Statement, while_statement_type: WhileStatementType) {
    parser_test(input, vec![
...
pub fn combined_while_until_test(input: &str, condition: Expression, then_block: Statement) {
    while_expression_test(input, condition.clone(), then_block.clone(), WhileStatementType::While);
    while_expression_test(input.replace("while", "until").as_str(), condition, then_block, WhileStatementType::Until);
}

pub fn combined_do_while_until_test(input: &str, condition: Expression, then_block: Statement) {
    while_expression_test(input, condition.clone(), then_block.clone(), WhileStatementType::DoWhile);
    while_expression_test(input.replace("while", "until").as_str(), condition, then_block, WhileStatementType::DoUntil);
}

pub fn for_statement_test(input: &str, variable_declarations: Vec<VariableDeclaration>, iterable: Expression, body: Statement) {
    parser_test(input, vec![
...
pub fn type_statement_test(type_str: &str, expected: Expression) {
    ...
    variable_declaration_test(&input, vec![
        let_variable(
            "x",
            opt_expr(expected),
            None
