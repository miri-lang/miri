// SPDX-License-Identifier: Apache-2.0
// Copyright (c) Viacheslav Shynkarenko

use super::utils::{parser_error_test, parser_test, variable_declaration_test};
use miri::ast::factory::{
    assign, call, class_identifier, expression_statement, func, identifier, let_variable,
    lhs_identifier, member, parameter, type_custom, type_expr_non_null, variable_statement,
};
use miri::ast::{opt_expr, AssignmentOp, MemberVisibility};
use miri::error::syntax::SyntaxErrorKind;

#[test]
fn test_namespaced_function_call() {
    parser_test(
        "Http::new(url)",
        vec![expression_statement(call(
            class_identifier("Http::new"),
            vec![identifier("url")],
        ))],
    );
}

#[test]
fn test_namespaced_enum_access() {
    parser_test(
        "let status = Http::Status.Ok",
        vec![variable_statement(
            vec![let_variable(
                "status",
                None,
                opt_expr(member(class_identifier("Http::Status"), identifier("Ok"))),
            )],
            MemberVisibility::Public,
        )],
    );
}

#[test]
fn test_namespaced_type_in_variable_declaration() {
    variable_declaration_test(
        "let client Http::Client",
        vec![let_variable(
            "client",
            opt_expr(type_expr_non_null(type_custom("Http::Client", None))),
            None,
        )],
        MemberVisibility::Public,
    );
}

#[test]
fn test_namespaced_type_in_function_return() {
    parser_test(
        "fn get_status() Http::Status: Http::Status.Ok",
        vec![func("get_status")
            .return_type(type_expr_non_null(type_custom("Http::Status", None)))
            .build(expression_statement(member(
                class_identifier("Http::Status"),
                identifier("Ok"),
            )))],
    );
}

#[test]
fn test_namespaced_type_in_function_parameter() {
    parser_test(
        "fn set_status(s Http::Status): _status = s",
        vec![func("set_status")
            .params(vec![parameter(
                "s".into(),
                type_expr_non_null(type_custom("Http::Status", None)),
                None,
                None,
            )])
            .build(expression_statement(assign(
                lhs_identifier("_status"),
                AssignmentOp::Assign,
                identifier("s"),
            )))],
    );
}

#[test]
fn test_error_namespaced_variable_declaration() {
    // A variable name cannot be namespaced.
    parser_error_test(
        "let Http::x = 1",
        &SyntaxErrorKind::UnexpectedToken {
            expected: "a simple identifier".to_string(),
            found: "Http::x".to_string(),
        },
    );
}

#[test]
fn test_error_namespaced_parameter_name() {
    // A function parameter name cannot be namespaced.
    parser_error_test(
        "fn my_func(Http::p int)",
        &SyntaxErrorKind::UnexpectedToken {
            expected: "a simple identifier".to_string(),
            found: "Http::p".to_string(),
        },
    );
}

#[test]
fn test_error_namespaced_assignment_target() {
    // A namespaced identifier like `Http::Status` is a value, not a variable,
    // so it cannot be the direct target of an assignment.
    parser_error_test(
        "Http::Status = 'new_status'",
        &SyntaxErrorKind::InvalidLeftHandSideExpression,
    );
}

#[test]
fn test_error_on_trailing_double_colon() {
    // A namespace path cannot end with `::`.
    parser_error_test("let x = MyModule::", &SyntaxErrorKind::UnexpectedEOF);
}
