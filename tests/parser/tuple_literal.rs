...
fn test_empty_tuple_unit_tuple() {
    parser_test(
        "let unit = ()",
        vec![variable_statement(
            vec![let_variable("unit", None, opt_expr(tuple(vec![])))],
            MemberVisibility::Public,
        )],
    );
}
    ...
    parser_test(
        ...
        vec![variable_statement(
            vec![let_variable(
                "num",
                None,
                opt_expr(tuple(vec![int_literal_expression(42)])),
            )],
            MemberVisibility::Public,
        )],
    );
    ...
    parser_test(
        ...
        vec![variable_statement(
            vec![let_variable(
                "num",
                None,
                opt_expr(tuple(vec![member(identifier("obj"), identifier("prop"))])),
            )],
            MemberVisibility::Public,
        )],
    );
    ...
    parser_test(
        ...
        vec![variable_statement(
            vec![let_variable(
                "num",
                None,
                opt_expr(tuple(vec![call(
                    member(identifier("obj"), identifier("func")),
                    vec![],
                )])),
            )],
            MemberVisibility::Public,
...
fn test_error_unclosed_tuple() {
    parser_error_test("let t = (1, 2", &SyntaxErrorKind::UnexpectedEOF);
}
...
fn test_tuple_index_precedence() {
    ...
    parser_test(
        ...
        vec![expression_statement(binary(
            index(
                tuple(vec![int_literal_expression(1), int_literal_expression(2)]),
                int_literal_expression(0),
            ),
            BinaryOp::Add,
            int_literal_expression(3),
