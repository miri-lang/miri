...
/// Execute Miri with multiple custom environment variables.
/// This overrides the default MIRI_LEAK_CHECK=1 with the supplied variables.
pub fn miri_run_with_env_multiple(input: &str, env_vars: &[(&str, &str)]) -> CompilerResult {
    use std::path::PathBuf;
    ...
    // Set provided environment variables (which may override defaults)
    for (key, value) in env_vars {
        cmd.env(key, value);
    }

    cmd.env_remove("MIRI_CC")
        .env_remove("CC")
        .arg("run")
        .arg(&path);
