...
fn test_number_edge_cases() {
    lexer_test("0 00 1_000_000 0.0 .5 5.", vec![
        Token::Int,
        Token::Int,
        Token::Int,
        Token::Float,
...
fn lexer_test(input: &str, expected: Vec<Token>) {
    ...
    let tokens: Vec<Token> = lexer.map(|(_start, token, _end)| token).collect();
