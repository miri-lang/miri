...
-   **`miri agent`**: Serves JSON-RPC 2.0 over stdin and stdout so one process answers many requests. `--help` carries the framing, the handshake and where the method list comes from, which is enough to write a client; [`docs/agent-protocol.md`](../../docs/agent-protocol.md) carries the whole protocol with a worked example per method. The session type-checks, explains, repairs, reads and edits source — it does not run, build or test a program, so an integration that has to execute what it compiled shells out.
