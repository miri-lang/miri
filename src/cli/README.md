...
-   **`miri fix <file.mi>`**: Reports the repairs the compiler recorded for a file (`--plan`, the default) or writes them (`--apply`). A repair classified as risky is refused unless `--allow-risky` is given. Every apply prints one envelope, whatever it did, so a consumer always has an `ok` to read; an apply that wrote nothing over an error no repair could mend reports `ok: false` and `MER_BLD_020`. With `--format json` the sentences written for a person go to standard error, leaving standard output to the envelope alone.
...
-   **`miri agent`**: Serves JSON-RPC 2.0 over stdin and stdout so one process answers many requests. `--help` carries the framing, the handshake and where the method list comes from, which is enough to write a client; [`docs/agent-protocol.md`](../../docs/agent-protocol.md) carries the whole protocol with a worked example per method. A message that is not a `Content-Length` frame — line-delimited JSON, say — is refused under `MER_BLD_024` on stderr and ends the session with exit status 1, so a client that framed wrongly is told rather than answered with silence. The session type-checks, explains, repairs, reads and edits source — it does not run, build or test a program, so an integration that has to execute what it compiled shells out.
...
-   **`miri test`**: Discovers `@test` functions in `.mi` files under a directory and runs each one in an isolated subprocess, with optional filtering by `<path>::<test_name>` and a selectable output format (pretty/json). A file that does not compile is refused once, with its errors under `not run:` and in the envelope's `diagnostics` carrying the `help` and `repair` a `check` of it gives; none of its tests reports a verdict, because none was built. A run that discovers no `@test` at all is refused under `MER_BLD_025` and exits 2 rather than reporting a passing suite of zero tests; the message says whether files were read and declared none or no `.mi` file was read, and `tests.filesRead` carries that count. Discovery, execution and reporting live in `src/test_runner/`; see its README.
