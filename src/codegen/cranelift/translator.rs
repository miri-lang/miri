...
/// Symbol prefix of the per-type element comparator a container sorts through.
///
/// The thunk it names takes two element values and answers which sorts first by
...
impl<'a> FunctionTranslator<'a> {
    ...
    /// Returns the address of `__compare_{type_name}` as a ptr-sized integer.
    ///
    /// Used as `elem_compare_fn` for List/Array holding elements whose bytes are
    /// a reference. The function is generated by `generate_compare_function`
    /// with Export linkage.
    pub(crate) fn get_custom_compare_thunk_addr(
        ...
        let compare_name = format!("{COMPARE_THUNK_PREFIX}{type_name}");
        let sig = Signature {
