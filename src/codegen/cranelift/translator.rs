...
impl<'a> FunctionTranslator<'a> {
    ...
    pub(crate) fn get_custom_clone_thunk_addr(
        ...
        let func_id = ctx
            ...
            .map_err(|e| format!("Failed to declare {clone_name}: {e}"))?;
        let local_func = ctx.module.declare_func_in_func(func_id, builder.func);
        Ok(builder.ins().func_addr(ptr_type, local_func))
    }
    ...
    pub(crate) fn call_rt_array_set_elem_clone_fn(
        ...
        Self::call_cached_func(
            builder,
            ctx.module,
            &mut ctx.rt_array_set_elem_clone_fn_id,
            rt::ARRAY_SET_ELEM_CLONE_FN,
            &[pt, pt],
            &[],
            &[array_ptr, fn_ptr],
        )?;
    ...
    pub(crate) fn call_rt_list_set_elem_clone_fn(
        ...
        Self::call_cached_func(
            builder,
            ctx.module,
            &mut ctx.rt_list_set_elem_clone_fn_id,
            rt::LIST_SET_ELEM_CLONE_FN,
            &[pt, pt],
            &[],
            &[list_ptr, fn_ptr],
        )?;
    ...
    pub(crate) fn call_rt_set_set_elem_clone_fn(
        ...
        Self::call_cached_func(
            builder,
            ctx.module,
            &mut ctx.rt_set_set_elem_clone_fn_id,
            rt::SET_SET_ELEM_CLONE_FN,
            &[pt, pt],
            &[],
            &[set_ptr, fn_ptr],
        )?;
    ...
    pub(crate) fn generate_drop_function(
        ...
        // Also generate __decref_TypeName: the RC-decrement wrapper used as
    ...
    fn generate_decref_function(
        ...
        let mut builder_ctx = FunctionBuilderContext::new();
        {
            ...
            // Load RC header and check for immortal (RC < 0)
            builder.switch_to_block(rc_block);
            builder.seal_block(rc_block);
            let header_ptr = builder.ins().iadd_imm(ptr, -ptr_size);
            let rc = builder.ins().load(
            ...
            // Decrement and conditional free
            builder.switch_to_block(dec_block);
            builder.seal_block(dec_block);
            let new_rc = builder.ins().iadd_imm(rc, -1);
            builder.ins().store(
        ...
        module
            .define_function(func_id, ctx)
            .map_err(|e| format!("Failed to define {decref_name}: {e}"))?;
    ...
    pub(crate) fn generate_clone_function(
        ...
        let mut builder_ctx = FunctionBuilderContext::new();
        {
            ...
            // null_ret_block: return null
            builder.switch_to_block(null_ret_block);
            builder.seal_block(null_ret_block);
            builder.ins().return_(&[null]);

            // call_block: delegate to the user's clone() method
            builder.switch_to_block(call_block);
            builder.seal_block(call_block);

            // Resolve the compiled name of clone(): walk inheritance to find
            // the defining class (applies the concrete-caller/abstract-definer rule).
            let clone_method_name = Self::resolve_clone_method_name(type_name, type_definitions);

            // Class methods have (self, allocator) ABI — pass 0 for the allocator.
            ...
            let user_clone_id = module
                ...
                .map_err(|e| format!("Failed to declare {clone_method_name}: {e}"))?;
        ...
        module
            .define_function(func_id, ctx)
            .map_err(|e| format!("Failed to define {clone_name}: {e}"))?;
        ...
        loop {
            match type_definitions.get(&current) {
                Some(TypeDefinition::Class(cd)) => {
                    if cd.traits.iter().any(|t| t == "Cloneable") {
                        return true;
                    }
                    match &cd.base_class {
