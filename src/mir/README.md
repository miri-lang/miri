...
-   **Symbols (`src/mir/symbol.rs`)**: Every body is lowered under a `Symbol` — what it stands for: a function at its type arguments, a method of its owner, a closure, a GPU kernel — and `Symbol::link_name` is the one place its name is spelled. A Miri definition is linked as `miri.` followed by `.`-separated segments, with argument tokens and compiler-synthesized markers led by `$`: `miri.pick$int` is `pick<int>`, `miri.pick__int` a function written `pick__int`, `miri.Point.norm` a method, `miri.Box$String.$drop` a drop function, `miri.$lambda7` a lambda. No identifier or type token contains `.` or `$`, so distinct symbols never share a link name, and none can take the name of the entry point `main` or of a `runtime` C function, both linked verbatim. GPU kernels, kernel data and string literals keep fixed compiler spellings, and `Symbol::wgsl_name` spells a body for WGSL with identifier characters only. The pipeline claims each symbol in a `SymbolTable` before lowering its body: claiming the same symbol again finds it already lowered, and a different symbol spelling a claimed name is refused with MER_MIR_018 as a guard over the grammar. Whether a call reaches a `runtime` function is what the declaration its name resolved to says (`TypeChecker::is_runtime_reference`), never its spelling.
