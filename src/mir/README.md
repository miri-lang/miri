...
-   **Symbols (`src/mir/symbol.rs`)**: Every body is lowered under a `Symbol` — what it stands for: a function at its type arguments, a method of its owner, a closure, a GPU kernel — and `Symbol::link_name` is the one place its name is spelled. The pipeline claims each symbol in a `SymbolTable` before lowering its body, so identity is the symbol, never the name: claiming the same symbol again finds it already lowered, while a different symbol that spells a name already claimed (a method `norm` of `Point` beside a function written `Point_norm`) refuses the program with MER_MIR_018 rather than running one body in place of the other.
