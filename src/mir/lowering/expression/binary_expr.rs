...
use crate::mir::{
    BinOp, Constant, Discriminant, Operand, Place, Rvalue, StatementKind as MirStatementKind, Terminator,
    TerminatorKind, UnOp,
};
...
pub(crate) fn lower_binary_expr(
    ...
    let is_option_eq = if let Some(lhs_ty) = ctx.type_checker.get_type(lhs.id) {
        matches!(&lhs_ty.kind, TypeKind::Option(_)) && matches!(op, crate::ast::operator::BinaryOp::Equal | crate::ast::operator::BinaryOp::NotEqual)
    } else { false };
    ...
    if is_option_eq {
        ...
        ctx.set_terminator(Terminator::new(TerminatorKind::Goto { target: final_bb }, expr.span));
        ...
        ctx.push_statement(crate::mir::Statement {
            kind: MirStatementKind::Assign(
                ...
                Rvalue::BinaryOp(BinOp::Eq, Box::new(lhs_op.clone()), Box::new(null_val.clone())),
            ),
            span: expr.span,
        });
        ...
        ctx.push_statement(crate::mir::Statement {
            kind: MirStatementKind::Assign(
                ...
                Rvalue::BinaryOp(BinOp::Eq, Box::new(rhs_op.clone()), Box::new(null_val.clone())),
            ),
            span: expr.span,
        });
        ...
        ctx.push_statement(crate::mir::Statement {
            kind: MirStatementKind::Assign(
                Place::new(result_local),
                Rvalue::BinaryOp(bin_op, Box::new(Operand::Copy(lhs_inner)), Box::new(Operand::Copy(rhs_inner))),
            ),
            span: expr.span,
        });
        ctx.set_terminator(Terminator::new(TerminatorKind::Goto { target: final_bb }, expr.span));
        ...
        let (target, ret_op) = if let Some(d) = dest {
            ...
            (Place::new(result_local), Operand::Copy(Place::new(result_local)))
        };
