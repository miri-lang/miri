...
4.  **Resumable**: `resume_at(offset)` restarts lexing at a byte offset, dropping the indentation stack, the open brackets, the buffered comments and any half-read formatted string. The parser calls it to pick up at a top-level declaration after a syntax fault, which is what lets one parse report faults in several declarations; clearing the bracket stack is what keeps a declaration that left a `(` open from swallowing the rest of the file. Spans stay absolute, so a diagnostic reported after a resume names the same byte a single-pass parse would have.
