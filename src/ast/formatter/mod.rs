...
//!
//! The rendering is canonical: it is derived from the AST, so the author's
//! spacing and indentation are normalized away. That is what lets a tool read a
...
//! the spelling of every number, and the spelling of every collection type —
//! `List<T>` stays `List<T>` and `[T]` stays `[T]`, since a constructor can only
//! be written the first way and a file that spelled one type both ways would be
//! the formatter's doing. The tree keeps neither spelling, so the renderer reads
//! them from the source it was parsed from; a tree rendered without its source
//! writes the sugar. `miri fmt` refuses to write a file when a word or a comment
//! did not survive, so a renderer that starts dropping one reports it rather
//! than deleting it.
//!
//! Comments and blank lines are rendered two ways. [`program`] keeps them — a
//! run of blank lines as one — so rewriting a file to its canonical text does
//! not cost the author their notes or their grouping. [`declaration`] drops
//! them, because that text is what an edit anchor is matched against: an anchor
//! able to match inside a comment could quietly edit the comment instead of the
//! code.
//!
