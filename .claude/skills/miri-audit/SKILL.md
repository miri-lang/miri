...
4. **Fix loop (skip if `--report-only`).** Dispatch the `lead-miri-engineer` with the consolidated critical + major findings (+ minor if `--with-minors`), by number, with file:line and the agreed fix, **plus an explicit file-scope contract** (only the files the findings name). It applies fixes via TDD where behavior changes. **Critical and major are never left undone.**
5. **Re-validate (tight — verify the engineer's claims yourself).** A subagent's "fixed" is a claim. Independently: run `git diff --name-only` and **reject any edit outside the scope contract** (route it back unless justified); then run the gate (or spawn `miri-test-runner`): `make format` (empty diff) → `make lint` (clean) → `make build` → `make test` (`cargo test --test mod`, exact counts) → `make audit` (clean for touched files). Read the actual output — never accept a "pre-existing / out-of-scope failure" excuse without re-running that exact test. If the engineer stalled or handed back, reclaim the fix into the main thread rather than re-dispatching into another watchdog window. **Re-run a specialist only if the fix changed code in its owned axis (§9) AND the fix was non-mechanical** — a mechanical fix (rename, extract, unwrap→`?`) is covered by `make audit` + tests, no re-panel.
...
- **Land the work: the orchestrator commits to `main` and pushes; subagents never touch git.** A task is not DONE while its changes sit in the working tree. Once the gate is green, the main thread stages **exactly the files touched, named explicitly** (never `git add -A`/`git add .`), commits in this repo's style (emoji + conventional prefix; a body saying *why*; the `Claude-Session:` trailer when the session gives one), pushes, and reports the SHA. Do not open PRs unless asked. **`git stash` is forbidden here** — stashing a path with no changes creates no entry, so the next `pop` pops the user's stash.
- **No subagent runs a git write command.** A dispatched agent may run `git diff`, `git status` and `git log` and nothing else — never `add`, `commit`, `push`, `stash`, `checkout`, `restore`, `reset` or `clean`. Say so explicitly in every dispatch prompt, and check `git status --short` yourself after each subagent returns: a *shrinking* diff means it wiped earlier work. Committing as soon as a chunk is green is the strongest protection — a committed tree cannot be wiped.
