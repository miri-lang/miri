...
3. **Implement with TDD, gated (MANDATORY — never skipped).** If this is a **bug fix**, first apply the root-cause-first protocol (AGENTS.md §4.1): reproduce as a failing test, trace the full pipeline path, and state the confirmed root cause with `file:line` evidence **before** editing — do not patch the first plausible symptom. Then, per acceptance criterion:
